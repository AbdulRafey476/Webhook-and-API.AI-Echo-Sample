  ...
  if (req.body.result.parameters.PrayerTime === "Time") {
    Request.get(`http://api.aladhan.com/v1/calendar?latitude=24.8667795&longitude=67.0311286&method=2&month=${now.getMonth()}&year=${now.getFullYear()}`, (err, response, body) => {
      if (err) throw err;
        ...
        var str = `<speak>Fajr time is ${timing.Fajr}, <break time="2s"/> Dhuhr time is ${timing.Dhuhr}, <break time="2s"/> Asr time is ${timing.Asr}, <break time="2s"/> Maghrib time is ${timing.Maghrib} <break time="3s"/> and Isha time is${timing.Isha}</speak>`;
        return res.json({
          speech: str,
          displayText: str,
